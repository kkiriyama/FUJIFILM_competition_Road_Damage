...
4. 次にYOLO v3の実装を始めた。[Kerasでの実装](https://github.com/qqwweee/keras-yolo3)をもとに今回のデータに合わせてファイルを改変した。Kerasを選択した理由は、バックエンドがtensorflowであるためChainerなどよりも高速であること、複雑なtensorflowをラップしているのでコードが簡潔であること、Google ColabolatoryのTPUの使用を見据えて簡単な変更でtensorflow.kerasを使用できるフレームワークであること、などである。画像サイズが32の倍数という指定だったのでとりあえずデフォルトの416 * 416にリサイズした。また、デフォルトのバッチサイズだと訓練のstage 2においてGPUのメモリ不足になってしまったのでstage2のバッチサイズを32から16に下げて学習を行った。学習に用いたGPUはAWSのインスタンス内のTesla K80である(以後同様)。
...
11. 以上の操作で得られた最高スコアである0.6455を正攻法でこの問題を解いた場合の最終提出スコアとした。予測時間は最初に測定したものと変わらず1枚当たり約0.15秒である。
...
- 今回のモデルを学習させるにあたり、最初はGoogle ColabolatoryのTPUを使用したいと考えていたが、今回使用したYOLO v3でのKerasによる実装で必要となる関数が`tensorflow.python.keras`のものと異なっていたため、重みを読み込むことができず断念した。TPUはGPUの数十倍のパフォーマンスを発揮する場合があるとのことなので、物体検出問題において時間があればtensorflowでの実装を探してTPU用に改変して使用することを考えたい。
- 複数のモデルのアンサンブルと画像サイズの拡大では通常スコアは上がると予想されるが予想に反する結果が得られた。これに関してはより詳細な分析を必要とする。例えば今回アンサンブルの方法として単純なnon max suppression を用いたが他の手法をリサーチして実装し、試す必要がある。画像サイズに関しては、デバッグする時間も十分に取れなかったのでまず入力画像サイズを変えたことによるconfigなどの
- 予測時間に関して、今回は画像1枚当たり約0.15秒となった。車載カメラで撮影すると仮定して30km/hで走行する車でのリアルタイムの予測をすると予測に用いられる各画像は約1.25mおきに撮影されることになる。これは訓練データの各画像を目視で確認する限り全ての道路を網羅するのに十分な制度であるといえる。